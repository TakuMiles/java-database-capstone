...
//    - Instruction: Ensure that this method handles edge cases, such as no prescriptions found for the given appointment, by returning meaningful responses.
    ...
    public ResponseEntity<String> getPrescriptionByAppointmentId(Long appointmentId) {
        try {
            // TODO: Implement database interaction
            // For now, return mock response
            return ResponseEntity.ok("Prescription found for appointment ID: " + appointmentId);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
